(function( $ ) {
	$.fn.minimap = function( $mapSource ) {
		...
		function init() {
			...
			$('.blackboard').attr('style','width:'+$mapSource[ 0 ].scrollWidth+'px');
			$mapSource.children().each( function() {
				var $child = $(this);
				var saveid= $(this).attr('id');
				var saveclass= $(this).attr('class');
				var saveattr= $(this).attr('disabled');
				var mini = $( "<div></div>" ).addClass( "minimap-node" ).addClass(saveid).addClass(saveclass).addClass(saveattr);
				$minimap.append( mini );
...
$(function () {
	var $parent = $('*[data-select="seat"]');
	$( "#minimap" ).minimap( $parent );
	$('*[data-select="seat"] button').click(function(){
		var saveClass = $(this).attr('id');
		$('*[data-select="seat"] button').removeClass('select');
		$(this).addClass('select');
		$('#minimap .minimap-node').removeClass('select');
		$('#minimap .'+saveClass).addClass('select');
	});
});