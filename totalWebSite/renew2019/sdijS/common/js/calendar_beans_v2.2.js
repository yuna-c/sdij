...
function CalAddCss(){
		...
		cssHtml += ".calDay{background-color: #FBFBFB;border:1px solid #CBC7BD;width:30px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;}";
		cssHtml += ".calSunDay{	background-color: #FBFBFB;border:1px solid #CBC7BD;	width:30px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;color:red;}";
		cssHtml += ".calSatDay{ background-color: #FBFBFB;border:1px solid #CBC7BD;	width:30px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;	color:blue;}";
		cssHtml += ".calOtherDay{background-color: #FBFBFB;	border:1px solid #CBC7BD;width:30px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;color:#CBC7BD;}";
		...
		cssHtml += ".dayEvMon{font-size:8pt;width:32px;	height:30px;background-color: #FBFBFB;border:1px solid #CBC7BD;	padding:4px 4px 0px 3px;float:left;cursor:pointer;}";		
		cssHtml += ".MonToday{font-size:8pt;width:32px;	height:30px;background-color: #368CFE;border:1px solid #CBC7BD;	font-weight:bold;padding:4px 4px 0px 3px;color:white;cursor:pointer;}";
		cssHtml += ".selMon{font-size:8pt;width:32px;height:30px;background-color: #CF4040;border:1px solid #CBC7BD;font-weight:bold;padding:4px 4px 0px 3px;text-decoration:underline;color:white;}";
		cssHtml += ".calOnDay{background-color: #006060;border:1px solid #4B22C0;float:left;cursor:pointer;color:white; }";
		cssHtml += ".calDaySpace{padding:1px;float:left; height:30px;}";
		cssHtml += ".calDayBr{clear:both;height:2px;}";
		cssHtml += ".calDayTitle{padding-top:2px;width:32px;float:left;	font-size:8pt; }";			
...
function initCal(obj){
	...
	var submitYn		= obj.submit  == null ? "n" : "y";
	var submitId		= obj.submitid
	var submitUrl		= obj.submitUrl == null ? "/" : obj.submitUrl ; 
	...
	$("#"+obj.id + ",#"+ calId + "Icon").click(function(event){					
		...
		if( calType == "day"){
			makeCal(calId,dates,splitKey,submitYn,submitId,submitUrl);
		}else{
			makeCalMon(calId,dates,splitKey,submitYn,submitId,submitUrl);
		}
...
//클릭 이벤트
function calClickEvent(id, submitYn, submitId, submitUrl){
	$("#" + id + " .calOtherDay,.calDay,.calSunDay,.calSatDay,.MonToday,.selMon,.dayEvMon").click(function(event) {
		...
		if (submitYn=="y")
		{
				$("#" + submitId).attr("action","" + submitUrl +"").submit();
		}
...
// 날짜변경(달변경) 달력내용 그리기
function makeCal(id,dates,splitKey,submitYn,submitId,submitUrl){				
	...
	calClickEvent(id, submitYn, submitId, submitUrl);
}

function makeCalMon(id,dates,splitKey,submitYn,submitId,submitUrl){
